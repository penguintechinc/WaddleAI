...
| Local chat / general generation | `gemma4:e4b` | **Default `e4b` everywhere; recommend `12b` in docs** for more complex operations (coding especially). `12b` is an opt-in (`WADDLEAI_LOCAL_CHAT_MODEL`), never a default that silently demands ~8GB VRAM |
